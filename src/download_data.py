...
def download_svarah(base_path):
    print("[LOG] Downloading Svarah Dataset from Hugging Face...")
    try:
        # Svarah on HF only has a 'test' split
        dataset = load_dataset("ai4bharat/Svarah", split="test")
        print("[SUCCESS] Svarah Loaded/Cached from Hugging Face (using 'test' split).")
        ...
        if not os.path.exists(repo_path):
            ...
            print("[SUCCESS] Svarah Repo Cloned (for scripts).")
    except Exception as e:
        print(f"[ERROR] Error downloading Svarah: {e}")

def download_indic_accent_db(base_path):
    print("[LOG] Downloading IndicAccentDb from Hugging Face...")
    try:
        dataset = load_dataset("DarshanaS/IndicAccentDb")
        print("[SUCCESS] IndicAccentDb loaded/cached.")
    except Exception as e:
        print(f"[ERROR] Error downloading IndicAccentDb: {e}")

def download_nptel2020(base_path):
    print("[WARNING] WARNING: NPTEL2020 is extremely large (15,700 hours, ~500GB+).")
    print("[LOG] Cloning NPTEL2020 repository for download scripts...")
    ...
    if not os.path.exists(target_path):
        ...
        print("[SUCCESS] NPTEL2020 repo cloned. Use scripts inside to download subsets.")
    else:
        print("[SKIP] NPTEL2020 repo already exists.")

def download_spire_sies(base_path):
    print("[LOG] Information for SPIRE-SIES...")
...
def download_accent_db(base_path):
    print("[LOG] Information for Accent DB...")
